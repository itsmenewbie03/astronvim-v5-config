...
function M.search_ancestors(startpath, func)
  if nvim_eleven then validate("func", func, "function") end
...
--- @return boolean
function M.path.is_dir(filename) return vim.fn.isdirectory(filename) == 1 end

--- @deprecated use `(vim.uv.fs_stat(path) or {}).type == 'file'` instead
--- @param path string
--- @return boolean
function M.path.is_file(path) return (vim.uv.fs_stat(path) or {}).type == "file" end
...
function M.path.join(...) return table.concat({ ... }, "/") end
...
function M.get_managed_clients()
  ...
  local clients = {}
  for _, config in pairs(configs) do
    if config.manager then vim.list_extend(clients, config.manager:clients()) end
  end
  return clients
end
...
function M.validate_bufnr(bufnr)
  if nvim_eleven then validate("bufnr", bufnr, "number") end
